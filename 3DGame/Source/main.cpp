...
#define WINDOW_NAME		"Robin"	// ウィンドウの名前
...
void DrawFPS(void)
{
	...
	nNum += sprintf(&aStr[nNum], "注視点の上下移動　：[T:上 B:下]\n");
	nNum += sprintf(&aStr[nNum], "視点の上下移動　　：[Y:上 N:下]\n");
	nNum += sprintf(&aStr[nNum], "ズームイン・アウト：[U:イン M:アウト]\n");
	nNum += sprintf(&aStr[nNum], "弾の発射　　　　　：[SPACE]\n");
