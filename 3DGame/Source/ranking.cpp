...
HRESULT InitRanking(void)
{
	...
	D3DXCreateTextureFromFile(
		pDevice,
		"data/TEXTURE/backtheme000.jpg",
		&g_pTextureRanking);

	// ロゴテクスチャの読み込み
	D3DXCreateTextureFromFile(
		pDevice,
		"data/TEXTURE/rankinglogo000.png",
		&g_pTextureRankingLogo);

	// 順位テクスチャの読み込み
	D3DXCreateTextureFromFile(
		pDevice,
		"data/TEXTURE/ranking_rank0.png",
		&g_pTextureRankingRank);

	// スコアテクスチャの読み込み
	D3DXCreateTextureFromFile(
		pDevice,
		"data/TEXTURE/number0000.png",
		&g_pTextureRankingScore);

	// 現在のスコアロゴテクスチャの読み込み
	D3DXCreateTextureFromFile(
		pDevice,
		"data/TEXTURE/yourscore0.png",
		&g_pTextureNowLogo);
		...
		if (g_aRankInScore[nCnt] == true)
		{
			for (int nCntScore = 0; nCntScore < MAX_SCORE; nCntScore++, pVtx = pVtx + 4)
			{
				...
				if (g_nColorCnt % 4 == 0 && g_nColorCnt % 8 != 0)
				{
					...
					// 各頂点カラーの設定
					pVtx[28].col = D3DXCOLOR(0.0f, fGreen, fBlue, 1.0f);
					pVtx[29].col = D3DXCOLOR(0.0f, fGreen, fBlue, 1.0f);
					pVtx[30].col = D3DXCOLOR(0.0f, fGreen, fBlue, 1.0f);
					pVtx[31].col = D3DXCOLOR(0.0f, fGreen, fBlue, 1.0f);
				}
			}
		}
		else
