...
void UpdateCamera(void)
{
	...
	// 目的の注視点
	g_camera.posRDest.x = player->pos.x - g_camera.fFront * sinf(player->rotDest);
	g_camera.posRDest.z = player->pos.z - g_camera.fFront * cosf(player->rotDest);
	g_camera.posRDest.y = player->pos.y + g_camera.fHeightR;

	// 目的の視点
	g_camera.posVDest.x = player->pos.x - sinf(g_camera.rot.y) * g_camera.fLength;
	g_camera.posVDest.z = player->pos.z - cosf(g_camera.rot.y) * g_camera.fLength - 150.0f;
	g_camera.posVDest.y = player->pos.y + cosf(g_camera.rot.x) * g_camera.fLength + g_camera.fHeightV + 150.0f;
