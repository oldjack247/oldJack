...
void SetVertexEnemy(int nIdx)
{
	...
	{
		//頂点座標
		pVtx[0].pos = D3DXVECTOR3(g_aEnemy[nIdx].pos.x - BIG_ENEMY_SIZEX, g_aEnemy[nIdx].pos.y + BIG_ENEMY_SIZEY, 0.0f);
		pVtx[1].pos = D3DXVECTOR3(g_aEnemy[nIdx].pos.x - BIG_ENEMY_SIZEX, g_aEnemy[nIdx].pos.y - BIG_ENEMY_SIZEY, 0.0f);
		pVtx[2].pos = D3DXVECTOR3(g_aEnemy[nIdx].pos.x + BIG_ENEMY_SIZEX, g_aEnemy[nIdx].pos.y + BIG_ENEMY_SIZEY, 0.0f);
		pVtx[3].pos = D3DXVECTOR3(g_aEnemy[nIdx].pos.x + BIG_ENEMY_SIZEX, g_aEnemy[nIdx].pos.y - BIG_ENEMY_SIZEY, 0.0f);
	}
	...
	for (int nCntEnemy = 0; nCntEnemy < MAX_ENEMY; nCntEnemy++, pEnemy++)
	{
		if (pEnemy->bUse == true)
		{
			...
			if (pPlayer->State == PLAYERSTATE_NORMAL)
			{
				...
				if (pEnemy->nType == 2)
				{
					if (pPlayer->pos.y + PLAYER_HITAREA > pEnemy->pos.y - BIG_ENEMY_SIZEX && pPlayer->pos.y - PLAYER_HITAREA < pEnemy->pos.y + BIG_ENEMY_SIZEY &&
						pPlayer->pos.x + PLAYER_HITAREA > pEnemy->pos.x - BIG_ENEMY_SIZEX && pPlayer->pos.x - PLAYER_HITAREA < pEnemy->pos.x + BIG_ENEMY_SIZEY)
					{
						HitPlayer(pPlayer->nLife);
					}
				}
...
bool HitEnemy(int nIdx, int nDamage)
{
	...
	if (g_aEnemy[nIdx].nLife <= 0)
	{
		...
		if (g_aEnemy[nIdx].nType == 2)
		{
			...
			//SetSubplayer(g_aEnemy[nIdx].pos);
		}
		else
		{
			g_nEnemyCount++;
