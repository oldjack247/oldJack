...
int g_nCounterWait;
int g_nCounterDeath;
int g_aIDSUBPLAYER[MAX_SUBPLAYER];
...
HRESULT InitPlayer(void)
{
	...
	for (int nCount = 0; nCount < SUBPLAYER_HAVE; nCount++)
	{
		g_aIDSUBPLAYER[nCount] = -1;
	}
...
void UpdatePlayer(void)
{
	...
	case PLAYERSTATE_NORMAL:
		...
		if (GetKeyboardTrigger(DIK_SPACE) == true)
		{
			...
			{
				if (g_aIDSUBPLAYER[nCount] != -1)
				{
					SetBulletSubplayer(g_aIDSUBPLAYER[nCount]);
				}
			}
		}
	...
	pSUBPLAYER = GetSubplayer();

	for (int nCntSUBPLAYER = 0; nCntSUBPLAYER < MAX_SUBPLAYER; nCntSUBPLAYER++, pSUBPLAYER++)
	{
		if (pSUBPLAYER->State == SUBPLAYERSTATE_APPEAR)
		{
			if (g_Player.pos.y + PLAYER_SIZEY > pSUBPLAYER->pos.y - SUBPLAYER_SIZE && g_Player.pos.y - PLAYER_SIZEY < pSUBPLAYER->pos.y + SUBPLAYER_SIZE &&
				g_Player.pos.x + PLAYER_SIZEY > pSUBPLAYER->pos.x - SUBPLAYER_SIZE && g_Player.pos.x - PLAYER_SIZEY < pSUBPLAYER->pos.x + SUBPLAYER_SIZE)
			{
				for (int nCntID = 0; nCntID < SUBPLAYER_HAVE; nCntID++)
				{
					if (g_aIDSUBPLAYER[nCntID] == -1)
					{
						g_aIDSUBPLAYER[nCntID] = nCntSUBPLAYER;
						break;
					}
				}
				if (g_aIDSUBPLAYER[SUBPLAYER_HAVE - 1] != -1)
				{
					AddScore(300);
					pSUBPLAYER->bUse = false;
				}

				pSUBPLAYER->State = SUBPLAYERSTATE_NORMAL;

			}
		}
	}

	if (g_aIDSUBPLAYER[0] != -1)
	{
		SetPosSubplayer(g_aIDSUBPLAYER[0], D3DXVECTOR3(g_Player.pos.x + 60, g_Player.pos.y, g_Player.pos.z));
	}

	if (g_aIDSUBPLAYER[1] != -1)
	{
		SetPosSubplayer(g_aIDSUBPLAYER[1], D3DXVECTOR3(g_Player.pos.x - 60, g_Player.pos.y, g_Player.pos.z));
	}
